...
## 3. Data Sharing
Volition Free sends negotiation content to `https://volition-free-api-jordansafers-projects.vercel.app/api/negotiate` for AI processing. My OpenAI API key mode sends requests directly to OpenAI or your configured endpoint. When enabled, automatic domain classification sends the domain and classification prompt to the selected provider. The extension does not use analytics or advertising trackers.
